...
Public Class ReportOutput
    ...
    Public Sub GenerateHTMLReport()
        ...
        HeaderPath = GetWorkingDirectoryPath("HTML\ReportHeader.html")
        ReportSummaryPath = GetWorkingDirectoryPath("HTML\ReportSummary.html")
        'Summary1Path = GetWorkingDirectoryPath("PagesSummary1.html")
        'Summary2Path = GetWorkingDirectoryPath("PagesSummary2.html")
        PageReportPath = GetWorkingDirectoryPath("HTML\PageReport.html")
        FooterPath = GetWorkingDirectoryPath("HTML\ReportFooter.html")
    ...
    Public Sub DisplayHTMLReport()
        Dim ReportPath As String = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) & "\Report.html"
        Dim RunResult As Boolean
        RunResult = Run(DefaultWebBrowser, ReportPath)

    End Sub
    Public Sub WriteHTMLReportOutToFile()
        Using output As StreamWriter = New StreamWriter(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) & "\Report.html", False)
    ...
    Public Function GetPageReport(ByRef Page As ReportOutputPage, ByRef PageReport As List(Of String)) As List(Of String)
        ...
        DifferenceQuantity = 0 'Page.OutputList.Count

        ParentPart = Page.ParentPart
        TotalCost = Page.TotalCost
        TotalLabor = Page.TotalLabor
        ...
        ReportCopy = SearchListForSubstituteString(ReportCopy, "{ParentPart}", ParentPart)
        ReportCopy = SearchListForSubstituteString(ReportCopy, "{TotalCost}", TotalCost)
        ReportCopy = SearchListForSubstituteString(ReportCopy, "{TotalLabor}", TotalLabor)
    ...
    Public Function GetTableReportRowList(ByRef Item As ReportMember, ByRef OC As String) As List(Of String)
        ...
        NewList.Add(OFT & "<tr>")
        NewList.Add(OFT & "    <td>" & Item.PartNo & "</td>")
        NewList.Add(OFT & "    <td>" & Item.Description & "</td>")
        NewList.Add(OFT & "    <td>" & Item.DrawingNumber & "</td>")
        NewList.Add(OFT & "    <td>" & Item.Quantity & "</td>")
        NewList.Add(OFT & "    <td>" & Item.Price & "</td>")
        NewList.Add(OFT & "    <td>" & Item.SupplierID & "</td>")
        NewList.Add(OFT & "    <td>" & Item.MakeBuy & "</td>")
        NewList.Add(OFT & "    <td>" & Item.Labour & "</td>")
        NewList.Add(OFT & "    <td>" & Item.MachineType & "</td>")
        NewList.Add(OFT & "</tr>")
        Return NewList
...
Public Class ReportMember
    ...
    Public Sub New(ByRef outputline As String)
        ...
        Dim tarr() As String
        tarr = Split(outputline, ",")
        PartNo = tarr(0)
        Description = tarr(1)
        DrawingNumber = tarr(2)
        Quantity = tarr(3)
        Price = tarr(4)
        SupplierID = tarr(5)
        MakeBuy = tarr(6)
